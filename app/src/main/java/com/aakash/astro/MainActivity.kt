...
class MainActivity : AppCompatActivity() {
    ...
    private fun openSpecialLagnas() {
        ...
        val intent = android.content.Intent(this, SpecialLagnasActivity::class.java).apply {
            putExtra(SpecialLagnasActivity.EXTRA_NAME, binding.nameInput.text?.toString())
            putExtra(SpecialLagnasActivity.EXTRA_EPOCH_MILLIS, birthDateTime.toInstant().toEpochMilli())
            putExtra(SpecialLagnasActivity.EXTRA_ZONE_ID, zone.id)
            putExtra(SpecialLagnasActivity.EXTRA_LAT, city.latitude)
            putExtra(SpecialLagnasActivity.EXTRA_LON, city.longitude)
        }
        startActivity(intent)
    }
